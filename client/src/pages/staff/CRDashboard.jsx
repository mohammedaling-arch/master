...
import {
    FileText, Bell, LogOut, LayoutDashboard, User, Menu, X, ClipboardList, MessageSquare
} from 'lucide-react';
import CRProbateReview from '../../components/staff/CRProbateReview';
...
const CRDashboard = ({ staff: initialStaff }) => {
    ...
    if (!staff) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading CR session...</div>;
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', color: '#3d2b1f', position: 'relative' }}>
                ...
                <div style={{ padding: '2rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'flex-start' : 'center', gap: '1rem' }}>
                    ...
                    {sidebarOpen && (
                        <div style={{ whiteSpace: 'nowrap', animation: 'fadeIn 0.2s' }}>
                            <h2 style={{ color: 'white', fontSize: '1.2rem', margin: 0 }}>CRMS CR</h2>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflowY: 'auto', width: '100%' }}>
                <Header
                    user={staff}
                    title={menus.find(m => m.id === activeTab)?.label || 'CR Overview'}
                ...
                <div style={{ padding: isMobile ? '1rem' : '2.5rem', flex: 1 }}>
                    {activeTab === 'dashboard' ? (
                        <RegistryAnalytics isMobile={isMobile} role="cr" onViewQueue={() => setActiveTab('review')} />
                    ) : activeTab === 'review' ? (
                        <CRProbateReview isMobile={isMobile} mode="cr_pending" title="Probate Approval Requests" />
                    ) : activeTab === 'letters' ? (
                        <CRProbateReview isMobile={isMobile} mode="history" title="Letter of Administration" />
                    ) : activeTab === 'support' ? (
                        ...
                        <div className="glass-card" style={{ background: 'rgb\(18 37 74\)', border: 'none', color: 'white' }}>
                            <h2>{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}</h2>
                            <p style={{ color: '#94a3b8', marginTop: '1rem' }}>Module view for "{activeTab}" is currently being implemented.</p>
                        </div>
                    )}
                </div>
            </main>
