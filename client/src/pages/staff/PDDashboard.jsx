...
import {
    ...
    LayoutDashboard, User, Menu, X, MessageSquare, CreditCard as PaymentIcon,
    History
} from 'lucide-react';
import PDPortal from '../../components/staff/PDPortal';
import Header from '../../components/common/Header';
import SupportTickets from '../../components/common/SupportTickets';
import StaffProfile from '../../components/staff/StaffProfile';
import ApplicantManager from '../../components/staff/ApplicantManager';

const PDDashboard = ({ staff: initialStaff }) => {
    ...
    const menus = [
        ...
        { icon: <Users size={20} />, label: 'Manage Applicants', id: 'applicants' },
        { icon: <FileText size={20} />, label: 'All Probate Filings', id: 'probates' },
        { icon: <History size={20} />, label: 'Letter of Admin', id: 'letters' },
        { icon: <PaymentIcon size={20} />, label: 'Payment Receipt', id: 'payments' },
        { icon: <MessageSquare size={20} />, label: 'Support', id: 'support' },
        { icon: <User size={20} />, label: 'My Profile', id: 'profile' },
    ];

    if (!staff) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Probate Desk session...</div>;
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', color: '#3d2b1f', position: 'relative' }}>
                ...
                <div style={{ padding: '2rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'flex-start' : 'center', gap: '1rem' }}>
                    ...
                    {sidebarOpen && (
                        <div style={{ whiteSpace: 'nowrap' }}>
                            ...
                            <p style={{ margin: 0, fontSize: '9px', color: '#8d6e63', fontWeight: 'bold' }}>PROBATE DESK</p>
                        </div>
                    )}
                </div>
                ...
                <div style={{ padding: '1rem' }}>
                    <button
                        ...
                        style={{
                            ...
                            gap: '1rem', padding: '1rem', background: 'transparent', border: 'none', color: '#ef4444',
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflowY: 'auto', width: '100%' }}>
                <Header
                    user={staff}
                    title={menus.find(m => m.id === activeTab)?.label || 'Probate Desk'}
                ...
                <div style={{ padding: isMobile ? '1rem' : '2.5rem', flex: 1 }}>
                    <PDPortal
                        activeTab={activeTab}
                        isMobile={isMobile}
                        staff={staff}
                        onTabChange={setActiveTab}
                        onUpdate={(updated) => { setStaff(updated); localStorage.setItem('staff', JSON.stringify(updated)); }}
                    />
                </div>
            </main>
        </div>
