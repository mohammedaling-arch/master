...
import JuratPortal from '../../components/staff/JuratPortal';
...
import JuratAnalytics from '../../components/staff/JuratAnalytics';

const JuratDashboard = ({ staff: initialStaff }) => {
    console.log('JuratDashboard staff:', initialStaff);
    ...
    const menus = [
        ...
        { icon: <FileText size={20} />, label: 'All Affidavits', id: 'affidavits' },
    ...
    if (!staff) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Jurat session...</div>;
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', color: '#3d2b1f', position: 'relative' }}>
                ...
                <div style={{ padding: '2rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'flex-start' : 'center', gap: '1rem' }}>
                    ...
                    {sidebarOpen && (
                        <div style={{ whiteSpace: 'nowrap', animation: 'fadeIn 0.2s' }}>
                            <h2 style={{ color: 'white', fontSize: '1.2rem', margin: 0 }}>CRMS Jurat</h2>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflowY: 'auto', width: '100%' }}>
                <Header
                    user={staff}
                    title={menus.find(m => m.id === activeTab)?.label || 'Jurat Overview'}
