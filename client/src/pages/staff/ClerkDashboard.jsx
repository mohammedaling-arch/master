...
import {
    ...
    LayoutDashboard, User, Menu, X, MessageSquare, CreditCard
} from 'lucide-react';
import Header from '../../components/common/Header';
import StaffProfile from '../../components/staff/StaffProfile';
import CFOAnalytics from '../../components/staff/CFOAnalytics';
import { AllAffidavitsTable, AllDeponentsTable } from '../../components/staff/CFODataTables';
import PaymentManager from '../../components/staff/PaymentManager';
import SupportTickets from '../../components/common/SupportTickets';

const ClerkDashboard = ({ staff: initialStaff }) => {
    ...
    const menus = [
        ...
        { icon: <CreditCard size={20} />, label: 'Payments & Receipts', id: 'payments' },
    ...
    if (!staff) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Clerk session...</div>;

    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', color: 'rgb(18 37 74)', position: 'relative' }}>
            {/* Mobile Overlay */}
                ...
                <div style={{ padding: '2rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'flex-start' : 'center', gap: '1rem' }}>
                    ...
                    {sidebarOpen && (
                        <div style={{ whiteSpace: 'nowrap', animation: 'fadeIn 0.2s' }}>
                            <h2 style={{ color: 'white', fontSize: '1.2rem', margin: 0 }}>CRMS Clerk</h2>
                    ...
                    {menus.map(menu => (
                        <div
                            ...
                            style={{
                                ...
                                cursor: 'pointer',
                                borderRadius: '12px',
                                background: activeTab === menu.id ? 'rgba(255,255,255,0.1)' : 'transparent',
                                color: activeTab === menu.id ? '#2ecc71' : '#cbd5e1',
                                marginBottom: '0.5rem',
                                transition: 'all 0.2s',
                                whiteSpace: 'nowrap'
                            }}
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflowY: 'auto', width: '100%' }}>
                <Header
                    user={staff}
                    title={menus.find(m => m.id === activeTab)?.label || 'Clerk Overview'}
                ...
                <div style={{ padding: isMobile ? '1rem' : '2rem', flex: 1 }}>
                    {activeTab === 'profile' ? (
                        ...
                        <PaymentManager isMobile={isMobile} />
                    ) : activeTab === 'deponents' ? (
